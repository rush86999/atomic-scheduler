...
*   **Endpoint**: `POST YOUR_SCHEDULER_HOST/timeTable/user/scheduleMeeting`
*   **Purpose**: Initiates a request to find a suitable time for a meeting for the specified participants, with a preference for a given date and time. This is an asynchronous operation. The scheduler will attempt to find a common slot and the result (success or failure, and the scheduled time if successful) will be communicated via the standard callback mechanism configured for `/solve-day`, using the `callBackUrl` provided in the underlying problem submission.
*   **Request Body**: `ScheduleMeetingRequest` (JSON object). See details below.
*   **Response**:
    *   HTTP `202 Accepted` if the request is well-formed and the scheduling process has been initiated. The body will contain a message and a `singletonId` for tracking.
    *   HTTP `400 Bad Request` if the input is invalid (e.g., participants not found, invalid date/time format).
    *   The actual scheduling solution (the found meeting time or failure) is delivered later via the main callback URL used by the solver, associated with the `singletonId` generated by this request.

## Input Data Structures

### `PostTableRequestBody`
...
When calling the `/scheduleMeeting` endpoint, your application must send a JSON object with the following structure:
